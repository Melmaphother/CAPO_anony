...
class RayAgentTrainer(RayPPOTrainer):
    ...
    def _prepare_validation_generation_dump(self, dump_path: str, total_trajectories: int) -> tuple[str, str]:
        """Create observable, append-only validation output for one validation run.

        The completed ``<step>.jsonl`` remains the public result.  While validation
        is running, each completed dataloader batch is fsynced to
        ``<step>.partial.jsonl`` and ``<step>.progress.json`` records its progress.
        A killed job therefore leaves an inspectable partial result rather than no
        evidence until the full validation set returns.
        """
        os.makedirs(dump_path, exist_ok=True)
        partial_path = os.path.join(dump_path, f"{self.global_steps}.partial.jsonl")
        progress_path = os.path.join(dump_path, f"{self.global_steps}.progress.json")
        with open(partial_path, "w"):
            pass
        self._write_validation_progress(progress_path, completed=0, total=total_trajectories, status="running")
        return partial_path, progress_path

    def _write_validation_progress(self, progress_path: str, completed: int, total: int, status: str) -> None:
        """Atomically publish validation progress so readers never see partial JSON."""
        payload = {
    ...
    def _append_validation_generations(
        self, inputs, outputs, gts, scores, reward_extra_infos_dict, partial_path: str
    ) -> None:
        """Durably append flattened validation agent steps to its partial JSONL."""
        n = len(inputs)
        base_data = {
            "input": inputs,
            "output": outputs,
        ...
        with open(partial_path, "a") as f:
            for index in range(n):
                entry = {key: _to_jsonable(values[index]) for key, values in base_data.items()}
                f.write(json.dumps(entry, ensure_ascii=False) + "\n")
            f.flush()
            os.fsync(f.fileno())

    def _finish_validation_generation_dump(
        self, partial_path: str, progress_path: str, completed: int, total: int
    ) -> None:
        """Atomically publish a complete validation dump under the historical name."""
        complete_path = partial_path.removesuffix(".partial.jsonl") + ".jsonl"
        os.replace(partial_path, complete_path)
        self._write_validation_progress(progress_path, completed=completed, total=total, status="completed")
        print(f"Dumped completed validation generations to {complete_path}")
    ...
    def _validate(self):
        ...
        total_validation_trajectories = len(self.val_dataset) * self.config.actor_rollout_ref.rollout.val_kwargs.n
        completed_validation_trajectories = 0
        if val_data_dir:
            partial_dump_path, progress_path = self._prepare_validation_generation_dump(
                val_data_dir, total_validation_trajectories
            )
        ...
        for test_data in self.val_dataloader:
            ...
            for n in num_steps:
                ...
                # Keep trajectory-level first/last samples for logger tables only.
                # The JSONL dump below intentionally uses every flattened agent step,
                # matching rollout_data_dir and preserving the full trajectory.
            ...
            if val_data_dir:
                step_inputs = self.tokenizer.batch_decode(
                    test_output_gen_batch.batch["prompts"], skip_special_tokens=True
                )
                ...
                step_gts = [
                    ground_truth
                    for ground_truth, n in zip(ground_truths, num_steps, strict=True)
                    for _ in range(n)
                ]
                assert len(step_inputs) == len(step_outputs) == len(step_scores) == len(step_gts), (
                    "validation step dump fields must have one entry per flattened agent step"
                )
                batch_dump_extra_infos = {"reward": step_scores.tolist(), **reward_extra_info}
                batch_dump_extra_infos.update(_collect_step_gae_dump_fields(test_output_gen_batch))
                self._append_validation_generations(
                    inputs=step_inputs,
                    outputs=step_outputs,
                    gts=step_gts,
                    scores=step_scores.tolist(),
                    reward_extra_infos_dict=batch_dump_extra_infos,
                    partial_path=partial_dump_path,
                )
                completed_validation_trajectories += len(batch_traj_scores)
                self._write_validation_progress(
                    progress_path,
                    completed=completed_validation_trajectories,
                    total=total_validation_trajectories,
                    status="running",
                )
        ...
        if val_data_dir:
            self._finish_validation_generation_dump(
                partial_path=partial_dump_path,
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    # DataProto slices every non-tensor field during padding and
                    # dispatch, so retain one driver-step value for each action row.
