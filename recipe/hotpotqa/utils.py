...
_DEFAULT_HOTPOTQA_CORPUS_DATA_ROOT = _PROJECT_ROOT / "data" / "corpus" / "hotpotqa_corpus"
...
class HotpotQASearchToolLegacy:
    ...
    the legacy retrieval helper used by this recipe.
    ...
    def batch_execute(self, args_list: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """Run one batched encode and search; on failure each row receives ``str(e)``."""
        if not args_list:
            return []
        try:
