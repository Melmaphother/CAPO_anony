...
def get_logger(save_dir: str, idx: int) -> logging.Logger:
    ...
    log_path = os.path.join(log_save_dir, f"paper_agent_{idx}.log")
    if os.path.exists(log_path):
        os.remove(log_path)

    logger = logging.getLogger(f"paper_agent_{idx}")
...
def get_thought_log_path(save_dir: str, idx: int) -> str:
    ...
    thought_log_path = os.path.join(thought_log_save_dir, f"paper_agent_{idx}.log")
...
def load_existing_ids(details_dir: str) -> set[int]:
    ...
    for fname in os.listdir(details_dir):
        if not (fname.startswith("paper_agent_") and fname.endswith(".json")):
            continue
        try:
        ...
        try:
            with open(test_file_path, "r", encoding="utf-8") as f:
                ...
                for idx, line in bar:
                    ...
                    save_path = os.path.join(details_dir, f"paper_agent_{idx}.json")
